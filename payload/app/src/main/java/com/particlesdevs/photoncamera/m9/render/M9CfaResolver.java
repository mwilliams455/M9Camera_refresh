...
public final class M9CfaResolver {
    ...
    public static boolean isSupported(int camera2Value) {
        return fromCamera2(camera2Value) != Pattern.UNSUPPORTED;
    }

    /** originX/originY are sensor-grid coordinates represented by local RAW (0,0). */
    public static Site colorAt(int localX, int localY, Pattern pattern, int originX, int originY) {
        if (pattern == null || pattern == Pattern.UNSUPPORTED) {
            throw new IllegalArgumentException("unsupported CFA pattern");
        }
    ...
    public static boolean isRedSite(int x, int y, Pattern p, int ox, int oy) {
        return colorAt(x, y, p, ox, oy) == Site.RED;
    }

    public static boolean isBlueSite(int x, int y, Pattern p, int ox, int oy) {
        return colorAt(x, y, p, ox, oy) == Site.BLUE;
    }

    public static boolean isGreen1Site(int x, int y, Pattern p, int ox, int oy) {
        return colorAt(x, y, p, ox, oy) == Site.GREEN_1;
    }
    ...
    public static String localPhaseName(Pattern p, int ox, int oy) {
        ...
        return symbol(colorAt(0, 0, p, ox, oy)) + symbol(colorAt(1, 0, p, ox, oy))
                + symbol(colorAt(0, 1, p, ox, oy)) + symbol(colorAt(1, 1, p, ox, oy));
    }

    private static String symbol(Site site) {
        switch (site) {
            case RED: return "R";
            case BLUE: return "B";
            default: return "G";
