...
required = [
    ...
    'PERF3B_COLOR8A', 'native_color_workers_4_to_8_scalar_parity',
    'tc20NativeLumaPopulationElapsedMs', 'nativeColorThreadOverheadApproxMsSum',
]
for token in required:
    if token not in renderer: raise SystemExit(f'missing renderer invariant {token}')
    print(f'OK renderer invariant {token}')

for token in ['workerWallStarted', 'inputCopyStarted', 'orientationStarted', 'totalWeightStarted', 'medianStarted', 'p98Started']:
    if token not in cpp: raise SystemExit(f'missing retained native probe {token}')
    print(f'OK retained probe {token}')
print('PERF3B COLOR8A source guard PASS')
