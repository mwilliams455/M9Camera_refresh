...
for token in [
    ...
    'PERF3H CVDIRECT1A',
    'reinterpret_cast<const jshort*>(static_cast<uintptr_t>(camAddress))',
]:
    if token not in cpp: raise SystemExit(f'missing native direct seam: {token}')
for token in [
    'renderBlockParallelDirect', 'meterTc20WeightedSelectDirect'
]:
    if token not in native_java: raise SystemExit(f'missing Java native declaration: {token}')
for token in [
    ('PERF3H_CVDIRECT1A_ORIENTFUSE8A_EXIFASYNC1A_JPEGBUF64K1A_TC20LUMA8A_COLOR8A' if 'PERF3H_CVDIRECT1A_ORIENTFUSE8A_EXIFASYNC1A_JPEGBUF64K1A_TC20LUMA8A_COLOR8A' in renderer else 'PERF3I_BITMAPDIRECT1A_CVDIRECT1A_ORIENTFUSE8A_EXIFASYNC1A_JPEGBUF64K1A_TC20LUMA8A_COLOR8A'),
    'opencv_mat_dataaddr_direct1a', 'java_short_array_fallback',
    'nativeColorCvDirectEligible', 'nativeColorCvDirectBlocks', 'nativeColorCvFallbackBlocks',
    'meterCvDirectEligible', 'meterCvDirectFallback',
    'cam16.isContinuous()', 'cam16.elemSize1() == 2L', 'cam16.step1() == (long)width * 3L',
    'meterCam16.isContinuous()', 'meterCam16.elemSize1() == 2L', 'meterCam16.step1() == (long)meterW * 3L',
    'JPEG_QUALITY = 95'
]:
    if token not in renderer: raise SystemExit(f'missing renderer seam: {token}')
# Direct path must be preservation-only: copied path remains as fallback.
for token in ['cam16.get(y0, 0, camBlock)', 'M9NativeColorCore.renderBlockParallel(', 'meterCam16.get(0, 0, meterCam)', 'tc20MeterNative(meterCam']:
    if token not in renderer: raise SystemExit(f'missing preservation fallback: {token}')
print('PERF3H CVDIRECT1A source guard PASS')
