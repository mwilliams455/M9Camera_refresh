...
## Installation correction: INSTALLFIX1

The first GL2G APK (SHA256 `fa74b3b0060e94f2c9159aeaf944e0b0cd461f89183be5e611b895c3c3c1d804`) was signed correctly but its compiled manifest contained a **1,039-character versionName**. GL2F contained 1,013 characters. AOSP `android.content.res.Element` limits `AndroidManifest_versionName` to `MAX_ATTR_LEN_NAME = 1024`, throwing a `SecurityException` for longer strings. See [the platform validator](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/main/core/java/android/content/res/Element.java). This is a concrete package defect consistent with the user's repeated "There's a problem with the app file" screenshot. Signing validation alone missed it; no on-device installer log was available.

INSTALLFIX1 replaces the accumulated history label with `1.61-m9livegl2g-installfix1` (27 characters). Only build metadata changes; all GL2G camera and rendering source hashes remain identical. The application ID, signing key and version code 26681 remain compatible with the preceding build. The output filename includes `installfix1` to distinguish it from the rejected artifact.

The new CI gate reads the **compiled binary manifest**, enforces Android's 1,024-character limit plus a conservative 64-character project limit, checks expected version/package/SDK/standalone metadata, verifies the APK signature and existing certificate, and checks ZIP/native-library alignment with the Android SDK tool. Its negative regression rejects the original GL2G APK specifically at `1039 > 1024`. Device installation and the existing photographic checks remain pending until the user tries the corrected build.
