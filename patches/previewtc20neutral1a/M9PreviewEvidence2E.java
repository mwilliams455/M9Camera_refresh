 ...
 * M9PREVIEWTC20NEUTRAL1A makes TC20 a neutral-reference scene property so
 * positive/negative exposure intent cannot move the tone predictor itself.
...
public final class M9PreviewEvidence2E {
    ...
    public static final String PARENT_TC20_REVISION="M9PREVIEWTC20NEG1A";
    public static final String TC20_REVISION="M9PREVIEWTC20NEUTRAL1A";
    ...
    /**
     * GL-thread call before the normal draw. TC20 is based only on the neutral
     * reference probe and changes by no more than 0.125 EV per fresh sample.
     */
    public float predictedTc20Gain(M9PreviewFrameState1W frame) {
        ...
        if(fresh) {
            ...
            // Fail open to unity; never carry a darkening decision across stale/foreign state.
            appliedTc20Ev=0.0;
    ...
    public void sample(M9PreviewFrameState1W frame,long textureNs,android.graphics.SurfaceTexture surface,
        ...
        try {
            ...
            for(int i=0;i<STEPS;i++){
                ...
                // Critical 1A change: TC20 follows the same neutral-reference domain
                // used by the still renderer's intent-normalized TC20. It does NOT
                // follow frame.exposureScale when Auto/user EV moves positive.
                GLES30.glUniform1f(exposureUniform,
                        (i==1||i==TC20_PANEL)?referenceScale:frame.exposureScale);
                // Preserve reference evidence at unity. Only the actual display panel
                // includes the current preview TC20 estimate. TC20 probe is pre-curve.
    ...
    public org.json.JSONObject snapshot(long shutterNs,M9ExposurePlan1A capture){
        ...
        try{
            o.put("revision",TC20_REVISION)
                    .put("parentPreviewTc20Revision",PARENT_TC20_REVISION)
                    .put("pairedEvidenceRevision",LEGACY_EVIDENCE_REVISION)
                    .put("available",false);
            Evidence e=latest;
            ...
            o.put("previewTc20NegativeOnly",true)
                    .put("previewTc20Domain","neutral_reference_not_display_intent")
                    ...
                    .put("previewTc20SlewEvPerSample",M9PreviewTc20Math1A.MAX_SLEW_EV)
                    .put("previewTc20DeadbandEv",M9PreviewTc20Math1A.DEADBAND_EV)
