...
public final class M9ShutterTrace1A {
    public static final String REVISION="M9SHUTTERTRACE1A";
    ...
    public static void thumbnail(android.content.Context context,android.net.Uri uri,Bitmap b) {
        try {
            ...
            submit(()->{try {
                ...
                Session s=session(LEDGER.byFilename(name));if(s==null)return;
                s.root.put("thumbnailDecode",M9ShutterRgb1A.json(copied)).put("thumbnailSourceUri",uri.toString());persist(s,"thumbnail_decode");
    ...
    private static JSONObject signature(M9JpegSignature1A.Result s)throws Exception {
        return new JSONObject().put("codingSha256",s.codingSha256).put("codingBytes",s.codingBytes)
    ...
    public static void begin(JSONObject preview,View view) {
        try {
            ...
            submit(()->{try {
                s.root.put("schema","m9cam.shuttertrace.v1a").put("revision",REVISION).put("captureId",id)
            ...
            MAIN.postDelayed(()->{
                if(e.completedNs<0){submit(()->{try{temporal(s,"still_result_timeout");}catch(Exception t){Log.w(REVISION,t.toString());}});LEDGER.close(id);}
            },60000);
    ...
    public static void completed(JSONObject preview,TotalCaptureResult result,String physicalId) {
        try {
            ...
            MAIN.postDelayed(()->submit(()->{try{temporal(s,"post_capture_window_complete");}catch(Exception t){Log.w(REVISION,t.toString());}finally{LEDGER.close(id);}}),6500);
        }catch(Throwable t){Log.w(REVISION,"completed failed: "+t);}
    }
    private static void temporal(Session s,String reason)throws Exception {
    ...
    public static void beforeEncode(Path path,Bitmap bitmap,CaptureResult result) {
        try {
            ...
            submit(()->{try{s.root.put("jpegFilename",path.getFileName().toString()).put("bitmapCopyNs",copyNs)
    ...
    public static void surface(String id,JSONObject evidence) {
        ...
        submit(()->{try {
            ...
            if(a.length()<24)a.put(evidence);else s.root.put("surfaceSamplesTruncated",true);
    ...
    private static void persist(Session s,String event)throws Exception {
        ...
        if(surfaces!=null)for(int i=0;i<surfaces.length();i++){JSONObject f=surfaces.optJSONObject(i);if(f!=null&&f.optInt("pixelCopyResult",-1)==0){gallery|="gallery".equals(f.optString("kind"));preview|="preview".equals(f.optString("kind"));}}
        coverage.put("galleryWindow",gallery).put("previewSurface",preview).put("allEndpointKindsRecorded",coverage.optBoolean("preEncode")&&coverage.optBoolean("jpegDecoded")&&coverage.optBoolean("postPreview")&&coverage.optBoolean("postMetadata")&&coverage.optBoolean("thumbnailDecoded")&&coverage.optBoolean("jpegMetadataRead")&&gallery&&preview);
