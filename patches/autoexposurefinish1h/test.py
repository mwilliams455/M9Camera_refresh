"""Compile AUTOEXPOSUREFINISH1H and exercise field ownership, fast acquisition, soft anchors and inherited M9 limits."""
...
public class PolicyTest {
 ...
 public static void main(String[] args){
  ...
  M9AutoExposure2D.reset();publish(dark());check(decide(0,0,0,0,true),.5,
      "large confident dark-scene deficit acquires by half stop");
  check(decide(0,0,0,0,true),.5,"reusing sample cannot ratchet exposure");
  publish(dark());check(decide(0,0,0,0,true),1.0,
      "fresh severe deficit permits the next half-stop acquisition step");
  ...
  check(decide(0,0,0,0,true),.50,
      "woodland dead-zone closes and begins with fast half-stop acquisition");
  publish(woods);
  check(decide(0,0,0,0,true),1.00,
      "woodland acquisition advances another half stop on a fresh rendered sample");
...
if assertions < 94: raise SystemExit(f'expected at least 94 assertions, got {assertions}')
...
receipt={
 'revision':'M9AUTOEXPOSUREFINISH1H_FIELDOWNERSHIP1A_FASTACQUIRE1A',
...
(build.parent/'autoexposurefinish1h_tests.json').write_text(json.dumps(receipt,indent=2)+'\n')
print(result,end='')
