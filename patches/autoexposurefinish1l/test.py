...
public class PolicyTest {
 ...
 // Reduced reproduction of the 2026-09-26 dark-bedding / bright-wall case.
 // A small coherent dark component is real, but the neutral-reference background
 // itself is not bright enough to justify the upper +1..+2.5 EV backlight range.
 static Stats[] openAnchorDeepBody(){
  Stats[] base=weakBackgroundDarkBody();
  Stats[] s=new Stats[base.length];int[] anchor={12,13,18};
  int[] am={164,178,191,203,215,226,236,244,251,255,255};
  int[] al={86,99,113,126,140,154,167,180,193,206,218};
  int[] ah={214,225,235,243,250,255,255,255,255,255,255};
  for(int i=0;i<base.length;i++){
   Stats q=base[i];
   int[] fm=q.fieldMedian.clone(),fq=q.fieldQ25.clone(),fh=q.fieldQ90.clone();
   double[] fb=q.fieldBright.clone(),fc=q.fieldClipped.clone();
   // Keep a connected 3-field textured open anchor. Field 12 carries the
   // strongest wall highlight, while 13 and 18 remain open texture.
   fm[12]=am[i];fq[12]=Math.min(am[i]-20,122+13*i);fh[12]=ah[i];
   fb[12]=i<1?.083333:(i<2?.111111:(i<3?.194444:(i<4?.305556:(i<5?.416667:.60))));
   fc[12]=i<3?0:(i==3?.055556:(i==4?.083333:.16));
   fm[13]=Math.min(236,110+14*i);fq[13]=Math.min(232,104+13*i);fh[13]=Math.min(246,128+14*i);
   fm[18]=Math.min(227,97+13*i);fq[18]=Math.min(218,86+13*i);fh[18]=Math.min(246,126+14*i);
   s[i]=gridStat(q.median,q.centerMedian,q.centerQ25,q.outerQ90,
       q.dark,q.bright,q.clipped,q.centerDark,q.outerBright,q.centerBright,
       q.centerClipped,q.outerClipped,fm,fq,fh,fb,fc);
  }return s;
 }

 static Stats[] weakBackgroundDarkBody(){
  int[] med={13,17,21,25,31,37,45,54,64,76,89};
  int[] cm ={10,12,16,19,24,29,35,43,51,61,72};
  int[] cq ={1,1,2,3,4,6,7,9,12,15,19};
  int[] oq ={85,98,111,125,138,152,166,179,192,205,217};
  int[] bmed={4,6,7,9,12,14,18,22,27,33,40};
  int[] blow={0,1,1,2,3,4,6,7,9,12,15};
  int[] bhi ={37,44,52,62,71,83,95,108,122,135,148};
  Stats[] s=new Stats[11];int[] body={9,10,11};
  for(int i=0;i<11;i++){
   int[] fm=ints(80+3*i),fq=ints(55+2*i),fh=ints(80+8*i);
   double[] fb=doubles(0),fc=doubles(0);
   fields(fm,fq,fh,fb,fc,body,bmed[i],blow[i],bhi[i],0,0);
   if(i==7)fields(fm,fq,fh,fb,fc,new int[]{12},180,160,220,.083,0);
   if(i==8)fields(fm,fq,fh,fb,fc,new int[]{12},194,175,234,.167,0);
   if(i==9){
    fields(fm,fq,fh,fb,fc,new int[]{12},207,188,242,.333,0);
    fields(fm,fq,fh,fb,fc,new int[]{18},180,160,220,.028,0);
   }
   if(i==10){
    fields(fm,fq,fh,fb,fc,new int[]{12},229,224,249,.778,.028);
    fields(fm,fq,fh,fb,fc,new int[]{13},225,223,229,.733,0);
    fields(fm,fq,fh,fb,fc,new int[]{14},209,199,222,.033,0);
    fields(fm,fq,fh,fb,fc,new int[]{18},205,196,225,.167,0);
   }
   double bright=i==10?.074:(i==9?.017:(i==8?.008:(i==7?.004:0)));
   double clip=i==10?.0013:0;
   s[i]=gridStat(med[i],cm[i],cq[i],oq[i],Math.max(.20,.62-.02*i),bright,clip,
       Math.max(.18,.54-.02*i),i>=7?.01:0,0,0,i==10?.0017:0,
       fm,fq,fh,fb,fc);
  }return s;
 }
 ...
 public static void main(String[] args){
  ...
  Stats[] weakBg=weakBackgroundDarkBody();
  BodyCandidate weakBgBody=M9AutoExposure2D.multifieldBodyCandidate(weakBg[0]);
  yes(weakBgBody.valid&&weakBgBody.fieldCount==3,
      "weak-background regression retains the genuine coherent dark body");
  check(M9AutoExposure2D.backlightEvidenceScore(weakBg[0],weakBgBody),0,
      "dim neutral-reference background contributes no upper-range backlight evidence");
  check(M9AutoExposure2D.backlightEvidenceLiftCap(weakBg[0],weakBgBody),1.0,
      "contrast alone may earn only the modest +1 EV backlight range");
  yes(M9AutoExposure2D.selectBacklight(weakBg)>=8,
      "underlying readability search still sees the severe subject deficit");
  check(M9AutoExposure2D.highlightRetentionLimit(weakBg),2.25,
      "emerging multi-field near-white stress retains one quarter stop of highlights");
  yes(!M9AutoExposure2D.highlightRetentionUnsafe(weakBg[0],weakBg[9])
          &&M9AutoExposure2D.highlightRetentionUnsafe(weakBg[0],weakBg[10]),
      "highlight retention triggers only when new broad near-white fields emerge");
  M9AutoExposure2D.reset();publish(weakBg);
  Decision weakFirst=M9AutoExposure2D.decide("2","PHOTO",now,1,0,0,0,0,true);
  check(weakFirst.appliedEv,.50,
      "weak-background severe body starts with bounded half-stop acquisition");
  yes(weakFirst.reason.equals("rendered_backlight_background_evidence_cap"),
      "weak-background decision records the evidence cap as the controlling reason");
  publish(weakBg);check(decide(0,0,0,0,true),1.0,
      "weak-background body converges to +1 EV rather than +2.5 EV");
  publish(weakBg);check(decide(0,0,0,0,true),1.0,
      "weak-background cap remains stable after convergence");

  Stats[] deepAnchor=openAnchorDeepBody();
  BodyCandidate deepAnchorBody=M9AutoExposure2D.multifieldBodyCandidate(deepAnchor[0]);
  yes(deepAnchorBody.valid&&deepAnchorBody.severity>=.85
          &&deepAnchorBody.bodyMedian<=18&&deepAnchorBody.bodyQ25<=8,
      "26-Sep style open-anchor scene contains a profoundly starved coherent body");
  yes(M9AutoExposure2D.protectedOpenAnchorFieldCount(deepAnchor[0])>=3,
      "26-Sep style wall forms a real protected textured open anchor");
  yes(!M9AutoExposure2D.protectedOpenAnchorSevereBacklight(deepAnchor[0],deepAnchorBody),
      "weak absolute background evidence keeps the scene out of severe-backlight anchor branch");
  yes(M9AutoExposure2D.protectedOpenAnchorDeepBodyAssist(deepAnchor[0],deepAnchorBody),
      "profoundly starved coherent body qualifies the 1L open-anchor middle tier");
  check(M9AutoExposure2D.backlightEvidenceLiftCap(deepAnchor[0],deepAnchorBody),1.0,
      "BGQUAL1A still limits weak-background authority to +1 EV");
  check(M9AutoExposure2D.protectedOpenAnchorLiftCap(deepAnchor[0],deepAnchorBody),.75,
      "deep-body open anchor now allows +0.75 EV instead of the old +0.25 EV pin");
  yes(M9AutoExposure2D.highlightRetentionLimit(deepAnchor)>=.75,
      "new +0.75 EV middle tier remains inside global highlight-retention safety");
  M9AutoExposure2D.reset();publish(deepAnchor);
  Decision deepFirst=M9AutoExposure2D.decide("2","PHOTO",now,1,0,0,0,0,true);
  check(deepFirst.appliedEv,.25,
      "deep-body open-anchor exposure still enters smoothly by a quarter stop");
  publish(deepAnchor);check(decide(0,0,0,0,true),.50,
      "deep-body open-anchor second fresh sample reaches +0.50 EV");
  publish(deepAnchor);
  Decision deepThird=M9AutoExposure2D.decide("2","PHOTO",now,1,0,0,0,0,true);
  check(deepThird.appliedEv,.75,
      "deep-body open-anchor converges to the +0.75 EV compromise ceiling");
  yes(deepThird.reason.equals("rendered_open_anchor_deep_body_cap"),
      "diagnostics identify the 1L open-anchor middle tier as the controlling cap");
  publish(deepAnchor);check(decide(0,0,0,0,true),.75,
      "deep-body open-anchor remains stable at +0.75 EV after convergence");
  ...
  // BODYLOCKRELEASE1A: a locally dark old mask must not survive forever when
  // the full current 4x6 detector no longer finds a coherent body.
  Stats[] dropout=new Stats[off.length];
  for(int i=0;i<off.length;i++){
   Stats q=off[i];
   dropout[i]=new Stats(110,q.dark,.20,q.clipped,
       q.centerMedian,q.centerQ25,q.outerQ90,
       q.centerDark,q.outerBright,q.centerBright,q.centerClipped,q.outerClipped,
       q.fieldMedian,q.fieldQ25,q.fieldQ90,q.fieldBright,q.fieldClipped);
  }
  yes(!M9AutoExposure2D.multifieldBodyCandidate(dropout[0]).valid,
      "current detector rejects high-key scene even though old mask pixels remain locally dark");
  M9AutoExposure2D.reset();publish(off);decide(0,0,0,0,true);
  int dropoutMask=M9AutoExposure2D.bodyLockMaskForDiagnostics();
  yes(dropoutMask!=0,"dropout regression starts from a valid latched body");
  publish(dropout);decide(0,0,0,0,true);
  yes(M9AutoExposure2D.bodyLockMaskForDiagnostics()==dropoutMask
          &&M9AutoExposure2D.bodyLockMissConfirmationsForDiagnostics()==1,
      "one fresh current-detector miss is tolerated without losing the body lock");
  decide(0,0,0,0,true);
  yes(M9AutoExposure2D.bodyLockMissConfirmationsForDiagnostics()==1,
      "reusing one meter sample cannot age out the body lock");
  publish(dropout);decide(0,0,0,0,true);
  yes(M9AutoExposure2D.bodyLockMaskForDiagnostics()==0,
      "second consecutive fresh current-detector miss releases the stale body lock");
...
if assertions < 125: raise SystemExit(f'expected at least 125 assertions, got {assertions}')
...
receipt={
 ...
 'body_candidate_absence_release_fresh_confirmations':2,
 'body_same_sample_reuse_does_not_age_lock':True,
 ...
 'protected_open_anchor_deep_body_max_ev':0.75,
 'protected_open_anchor_deep_body_revision':'OPENANCHORBAL1A',
 'protected_open_anchor_deep_body_requires_weak_bgqual':True,
 ...
 'background_qualification_revision':'BGQUAL1A',
 'weak_background_max_ev':1.0,
 'upper_backlight_range_requires_absolute_bright_background_evidence':True,
 'highlight_retention_revision':'HIGHLIGHTRET1A',
 'highlight_retention_policy':'global_single_capture_exposure_incremental_emerging_highlights_no_HDR',
 'highlight_retention_field_map_required':True,
 'highlight_retention_nearwhite_fields_threshold':2,
 'highlight_retention_local_hdr':False,
 'highlight_retention_multiframe_hdr':False,
...
(build.parent/'autoexposurefinish1l_tests.json').write_text(json.dumps(receipt,indent=2)+'\n')
print(result,end='')
