...
/** Read-only 1:1 viewport crops at 1 Hz, bounded to 20 frames. Never feeds Auto. */
public final class M9RootCausePixels1A {
    private static final long INTERVAL_NS=1000000000L;
    private static final int W=96, H=96;
    private static final int STEPS=3, BYTES=W*H*STEPS*4;
    ...
    public void sample(M9PreviewFrameState1W frame,long textureNs,android.graphics.SurfaceTexture surface,int exposureUniform,
        ...
        try {
            ...
            for(int i=0;i<STEPS;i++) {
                ...
                GLES30.glViewport(i*W-(viewport[2]-W)/2,-(viewport[3]-H)/2,viewport[2],viewport[3]);
                GLES30.glUniform1i(evidenceUniform,i==0?1:0);
            ...
            pendingViewport=viewport.clone();
        ...
        try {
            ...
            if(now-pendingNs<=2500000000L) {
                latest=new Evidence(pending,pendingNs,pendingTextureNs,pendingDataSpace,
                        pendingReferenceScale,pixels,pendingViewport);
                synchronized(history) {
                    while(!history.isEmpty()&&(history.size()>=20||now-history.peekFirst().submittedNs>20000000000L))history.removeFirst();
    ...
    public org.json.JSONObject snapshot(long shutterNs,M9ExposurePlan1A capture) {
        ...
        try {
            ...
            if(capture==null)return o.put("reason","no_capture_plan");
            ...
            for(Evidence e:copy) {
                if(!e.state.plan.cameraId.equals(capture.cameraId)||!e.state.plan.mode.equals(capture.mode))continue;
                double age=(shutterNs-e.submittedNs)/1e6;
                if(age<0||age>20000)continue;
                M9ExposurePlan1A p=e.state.plan;
                org.json.JSONObject f=new org.json.JSONObject();
                f.put("submittedElapsedNs",e.submittedNs).put("textureTimestampNs",e.textureNs)
                ...
                for(int i=0;i<STEPS;i++)panels.put(names[i],panel(e.rgba,i));
            ...
            return o.put("available",frames.length()>0).put("reason",frames.length()>0?"history_available":unavailableReason)
                .put("frames",frames).put("width",W).put("height",H).put("capacity",20).put("intervalMs",1000)
                .put("crop","center_96x96_at_original_GL_viewport_pixel_scale; not_sensor_pixel_scale")
