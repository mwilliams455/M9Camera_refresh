...
with tempfile.TemporaryDirectory() as tmp:
 ...
 for rel in ['m9/M9ExposurePlan1A','m9/preview/M9AutoExposure2D','m9/preview/M9PreviewMeter2D']:
  (d/(rel.rsplit('/',1)[-1]+'.java')).write_text((j/(rel+'.java')).read_text())
 text=(d/'M9PreviewMeter2D.java').read_text()
 ...
 code+='''
 public static int draws,reads,maps,peaking=1;static int id=100,draw=3,read=4,pack=7,active=0;
 static int[] viewport={2,3,1080,1920};static Map<Integer,Integer> tex=new TreeMap<>();
 ...
 static ByteBuffer bytes=ByteBuffer.allocateDirect(32*24*7*4);
 static {tex.put(GL_TEXTURE3,97);for(int y=0;y<24;y++)for(int s=0;s<7;s++)for(int x=0;x<32;x++){int v=10+s*10;bytes.put((byte)v).put((byte)v).put((byte)v).put((byte)255);}bytes.position(0);}
 public static String snapshot(){return draw+":"+read+":"+pack+":"+active+":"+tex+":"+Arrays.toString(viewport)+":"+scissor+":"+uniforms+":"+peaking;}
 ...
 public static void glUniform1i(int k,int v){peaking=v;}public static void glUniform1f(int k,float v){uniforms.put(k,v);}
 public static void glViewport(int x,int y,int w,int h){viewport=new int[]{x,y,w,h};}
 public static void glDrawArrays(int mode,int first,int count){draws++;if(count!=4)throw new AssertionError();}
 public static void glBindBuffer(int target,int value){pack=value;}
 public static void glReadPixels(int x,int y,int w,int h,int format,int type,int offset){reads++;if(w!=224||h!=24||offset!=0)throw new AssertionError();}
 ...
 public static void glBufferData(int a,int bytes,Buffer data,int usage){if(bytes!=21504)throw new AssertionError();}
 ...
 (d/'M9GpuPreview2A.java').write_text('package com.particlesdevs.photoncamera.m9.preview;public class M9GpuPreview2A {public static class Frame {public final boolean ready;public final String cameraId;public Frame(boolean r,String c){ready=r;cameraId=c;}}}')
 ...
 ''')
 (d/'MeterTest.java').write_text((here/'MeterTest.java').read_text())
 ...
 subprocess.run(['java','-ea','-cp',str(out)+':'+str(jar),'MeterTest'],check=True)
