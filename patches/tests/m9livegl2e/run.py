...
with tempfile.TemporaryDirectory() as tmp:
 ...
 for rel in ['m9/M9ExposurePlan1A','m9/preview/M9AutoExposure2D','m9/preview/M9PreviewEvidence2E']:
  (d/(rel.rsplit('/',1)[-1]+'.java')).write_text((j/(rel+'.java')).read_text())
 text=(d/'M9PreviewEvidence2E.java').read_text()
 ...
 static ByteBuffer bytes=ByteBuffer.allocateDirect(32*24*3*4);
 static {tex.put(GL_TEXTURE3,97);for(int y=0;y<24;y++)for(int s=0;s<3;s++)for(int x=0;x<32;x++){int v=10+s*30;bytes.put((byte)v).put((byte)v).put((byte)v).put((byte)255);}bytes.position(0);}
 public static String snapshot(){return draw+":"+read+":"+pack+":"+active+":"+tex+":"+Arrays.toString(viewport)+":"+scissor+":"+uniforms+":"+peaking+":"+stage;}
 ...
 public static void glDrawArrays(int mode,int first,int count){draws++;stages.add(stage);scales.add(uniforms.get(10));if(count!=4)throw new AssertionError();}
 public static void glBindBuffer(int target,int value){pack=value;}
 public static void glReadPixels(int x,int y,int w,int h,int format,int type,int offset){reads++;if(w!=96||h!=24||offset!=0)throw new AssertionError();}
 ...
 public static void glBufferData(int a,int bytes,Buffer data,int usage){if(bytes!=9216)throw new AssertionError();}
