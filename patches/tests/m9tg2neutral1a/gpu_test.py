...
with tempfile.TemporaryDirectory() as tmp:
 ...
 '''+fn+'''extern "C" void oracle(const float* input,int n,const uint8_t* curve,double tg,uint8_t* out){ColorContext ctx;std::copy(curve,curve+2048,ctx.curve.begin());for(int i=0;i<n;i++){double m[]={input[4*i],input[4*i+1],input[4*i+2]};int rgb[3];m9CurvePixel(m,1,ctx,rgb);int r=rgb[0],g=rgb[1],b=rgb[2];int y=(4899*r+9617*g+1868*b)>>14;int cb=((-2765*r-5427*g+8192*b)>>14);int cr=((8192*r-6860*g-1332*b)>>14);cb=((cb+128)&255)-128;cr=((cr+128)&255)-128;auto sm=[](double a,double b,double x){if(x<=a)return 0.0;if(x>=b)return 1.0;double t=(x-a)/(b-a);return t*t*(3.0-2.0*t);};double cbm=cb,crm=cr;double chroma=std::sqrt(cbm*cbm+crm*crm);double neutral=1.0-sm(38.0,78.0,chroma);double yg=sm(2.0,22.0,-cbm);double og=yg*sm(2.0,20.0,crm);double gg=sm(2.0,18.0,-crm);if(cbm<0)cbm*=1.0-tg*yg*((1.0-neutral)*.18+neutral*.58);if(crm>0)crm*=1.0-tg*og*(.42*neutral);else if(crm<0)crm*=1.0-tg*gg*((1.0-neutral)*.10+neutral*.22);double v[]={y+1.402*crm,y-.344136*cbm-.714136*crm,y+1.772*cbm};for(int k=0;k<3;k++)out[3*i+k]=(uint8_t)(std::max(0.,std::min(255.,v[k]))+.5);}}''')
...
report={'revision':'M9TG2NEUTRAL1A','renderer':gs(0x1F01).decode(),'version':gs(0x1F02).decode(),'pixelsPerTungstenState':count,'tg2NeutralOracleComparisons':results,'fullProgramAndFallbackBracket':'PASS','deviceParityOrCadenceVerified':False}
