...
public class EvidenceTest {
 ...
 public static void main(String[] args)throws Exception {
  ...
  check(GLES30.crops.get(0).equals("[-238, -928, 1080, 1920]|[0, 0, 64, 64]"),"native viewport crop, not resized full image");
  check(GLES30.crops.get(2).equals("[-110, -928, 1080, 1920]|[128, 0, 64, 64]"),"third panel placement and isolation");
  check(GLES30.stages.toString().equals("[1, 0, 0, 1, 0, 0, 1, 0, 0]"),"actual source/reference/display stages");
  ...
  check(GLES30.maps==1&&GLES30.draws==9,"readback later; one Hz throttle");
  ...
  for(int i=0;i<9;i++) {
   JSONObject panel=first.getJSONObject("panels").getJSONObject("region"+(i/3)+"_"+new String[]{"incomingOes","referenceRender","displayRender"}[i%3]);
   ...
   check(rgb.length==64*64*3,"lossless RGB payload length");
   for(byte v:rgb)if((v&255)!=10+i*20)throw new AssertionError("atlas channel layout");
  }
  ...
  check(p.snapshot(SystemClock.now,f.plan).getJSONArray("frames").length()<=48,"bounded history");
  ...
  check(p.range(0,SystemClock.now,"4").getBoolean("available"),"post-shutter range remains accessible");
  M9RootCausePixels1A fast=new M9RootCausePixels1A();GLES30.complete=true;M9ShutterTrace1A.active=true;
  int before=GLES30.draws;SystemClock.now+=1000000000L;long shutter=SystemClock.now;
  fast.sample(f,1,null,10,11,1,12);SystemClock.now+=10000000L;fast.sample(f,1,null,10,11,1,12);
  SystemClock.now+=250000000L;fast.sample(f,2,null,10,11,1,12);SystemClock.now+=10000000L;fast.sample(f,2,null,10,11,1,12);
  check(GLES30.draws-before==18,"4Hz capture-window sampling");
  check(fast.range(shutter+1,SystemClock.now,"4").getJSONArray("frames").length()==1,"post-only range excludes shutter/pre samples");
  check(state.equals(GLES30.snapshot()),"accelerated sampling leaves display state unchanged");
  TraceTest.run();System.out.println("M9SHUTTERTRACE1A COLLECTOR PASS: "+n+" GPU-collector checks plus metadata checks");
 }
}
