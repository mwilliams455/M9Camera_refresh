...
public class TraceTest {
 ...
 public static void run()throws Exception {
  ...
  physical.put(CaptureResult.COLOR_CORRECTION_TRANSFORM,new android.hardware.camera2.params.ColorSpaceTransform());
  physical.put(CaptureResult.TONEMAP_CURVE,new android.hardware.camera2.params.TonemapCurve());
  ...
  JSONObject context=snap.getJSONArray("contexts").getJSONObject(0).getJSONObject("context");
  check(context.getJSONArray("ccmRowMajor").length()==9,"actual frame CCM saved");
  check(context.getJSONArray("toneCurvesRgb").length()==3,"actual frame tone curves saved");
  ...
  check(M9RootCauseTrace1A.range(1000000001L,SystemClock.now,"0-4").getJSONArray("rows").length()==1800,"post-shutter metadata retained");
