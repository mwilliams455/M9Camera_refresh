...
with tempfile.TemporaryDirectory() as tmp:
 ...
 static ByteBuffer bytes=ByteBuffer.allocateDirect(64*64*9*4);
 static {tex.put(GL_TEXTURE3,97);for(int y=0;y<64;y++)for(int s=0;s<9;s++)for(int x=0;x<64;x++){int v=10+s*20;bytes.put((byte)v).put((byte)v).put((byte)v).put((byte)255);}bytes.position(0);}
 ...
 public static void glReadPixels(int x,int y,int w,int h,int format,int type,int offset){reads++;if(w!=576||h!=64||offset!=0)throw new AssertionError();}
 ...
 public static void glBufferData(int a,int bytes,Buffer data,int usage){if(bytes!=147456)throw new AssertionError();}
 ...
 public static class Draw {public Draw(M9PreviewFrameState1W f,long t,long n,long s,boolean b){} public String snapshot(long n,M9ExposurePlan1A p){return "{}";}}}

 ''')
 (d/'M9ShutterTrace1A.java').write_text('package com.particlesdevs.photoncamera.m9.preview;public class M9ShutterTrace1A {public static boolean active;public static boolean sampling(){return active;}}')
