...
with tempfile.TemporaryDirectory(prefix='m9-exposure-test-') as d:
 ...
 write(P+'m9.M9Config','public class M9Config {public static boolean pipeline=true;public static boolean usesM9Pipeline(){return pipeline;}public static boolean isM9Modern(){return true;}public static boolean isCaptureTest(){return true;}}')
 ...
 public boolean isDualSession=false;PLACEHOLDER_IS_ZSLpublic void resetPreviewAEMode(){}public void rebuildPreviewBuilder(){}public void unlockFocus(){}}
 ''')
 ...
 ''')
 # Compile the actual production routing method; the old test's constant false
 # stub could not detect Motion silently taking the ZSL bypass.
 captureSource=(J/'capture/CaptureController.java').read_text()
 begin=captureSource.index('    public boolean isZslMode() {')
 end=captureSource.index('\n    private void triggerZslCapture()',begin)
 method=captureSource[begin:end].strip()
 captureStub=src/(P+'capture.CaptureController').replace('.','/')
 captureStub=captureStub.with_suffix('.java')
 stub=captureStub.read_text().replace('PLACEHOLDER_IS_ZSL',method)
