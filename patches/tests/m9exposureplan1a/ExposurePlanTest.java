    ...
    public static void main(String[] args) {
        ...
        for(boolean backlit:new boolean[]{false,true}) {
            ...
            for(int steps:new int[]{-3,-1,0,1,3}) {
                ...
                check(p.matches("0","PHOTO",1001,p.userEv,0,0),"fresh matching plan usable");
                check(!p.matches("1","PHOTO",1001,p.userEv,0,0),"lens change invalidates");
                check(!p.matches("0","NIGHT",1001,p.userEv,0,0),"mode change invalidates");
                check(!p.matches("0","PHOTO",2601,p.userEv,0,0),"stale plan invalidates");
                check(!p.matches("0","PHOTO",1001,p.userEv+1,0,0),"EV change invalidates");
                check(!p.matches("0","PHOTO",1001,p.userEv,1000000,0),"shutter change invalidates");
                check(!p.matches("0","PHOTO",1001,p.userEv,0,200),"ISO change invalidates");
        ...
        System.out.println("M9EXPOSUREPLAN1A HOST PASS: "+checks+" assertions; real allocator/MFM/manual controls, synthetic scenes, no device claim");
    }
}
