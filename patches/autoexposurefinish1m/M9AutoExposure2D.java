...
public final class M9AutoExposure2D {
    public static final String REVISION="M9AUTOEXPOSUREFINISH1M_HIGHLIGHTGRANDFATHER1A";
    ...
    // 1G: an already-open textured anchor no longer vetoes a coherent dark body.
    // It limits the additional positive Auto lift instead: a quarter stop normally,
    // or a half stop when strong bright-background evidence accompanies severe starvation.
    private static final double PROTECTED_ANCHOR_ORDINARY_MAX_EV=.25;
    private static final double PROTECTED_ANCHOR_SEVERE_MAX_EV=.50;
    // 1L: if the current coherent body is profoundly starved, a textured open
    // anchor no longer pins exposure to +0.25 EV. A middle tier permits +0.75 EV,
    // while BGQUAL1A and HIGHLIGHTRET1A remain independent upper bounds.
    private static final double PROTECTED_ANCHOR_DEEP_BODY_MAX_EV=.75;

    // 1K: a dark coherent component alone may justify a modest lift, but the
    // upper backlight range now requires absolute bright-background evidence.
    // This prevents dark bedding/furniture from earning +2.5 EV merely because
    // it is much darker than the rest of an otherwise dim frame.
    private static final double WEAK_BACKGROUND_MAX_EV=1.00;
    // 1M: a field whose neutral-reference q90 is already within four code values
    // of the >=224 near-white threshold is existing highlight structure, not a
    // newly-created highlight. Incremental field-loss accounting therefore
    // grandfathers it, while global growth and genuinely emerging fields remain
    // hard capture-exposure bounds.
    private static final int PREEXISTING_HIGHLIGHT_Q90_CODE=220;
    // 1H: keep 0.25-EV fine placement, but permit a 0.50-EV acquisition step
    // when the rendered meter shows a large, high-confidence exposure deficit.
    private static final double NORMAL_POSITIVE_SLEW_EV=.25;
    private static final double FAST_POSITIVE_SLEW_EV=.50;
    // 1I: optional 0.75-EV first/early acquisition step. This never bypasses
    // the rendered target or positive headroom limit; it is only allowed with
    // a very fresh sample, a >=1.25-EV remaining gap, and severe starvation.
    private static final double SAFE_FAST_POSITIVE_SLEW_EV=.75;
    ...
    // current rendered evidence remains coherent. A disjoint replacement needs
    // two fresh confirmations; two consecutive fresh candidate-absence samples
    // release an otherwise locally-plausible stale mask.
    private static int latchedBodyMask;
    private static int latchedBodyTargetMedian,latchedBodyTargetQ25;
    private static double latchedBodySeverity;
    private static int pendingBodyMask,pendingBodyConfirmations,bodyMissConfirmations;
    private static String bodyLockReason="none";
    ...
    public static synchronized int bodyLockMissConfirmationsForDiagnostics() {
        return bodyMissConfirmations;
    }
    ...
    public static int selectSceneKey(Stats[] s) {
        ...
        BodyCandidate body=s[0].fieldMapValid
                ?multifieldBodyCandidate(s[0]):BodyCandidate.invalid("field_map_unavailable");
        return selectSceneKeyQualified(s,body);
    }

    private static int selectSceneKeyQualified(Stats[] s,BodyCandidate body) {
        if(!validStats(s)||!sceneKeyStarved(s[0],body))return 0;
    ...
    public static double sceneKeyHeadroomLimit(Stats[] s) {
        ...
        BodyCandidate body=s[0].fieldMapValid
                ?multifieldBodyCandidate(s[0]):BodyCandidate.invalid("field_map_unavailable");
        return sceneKeyHeadroomLimitQualified(s,body);
    }

    private static double sceneKeyHeadroomLimitQualified(Stats[] s,BodyCandidate body) {
        if(!validStats(s)||!sceneKeyStarved(s[0],body))return 0;
        Stats base=s[0];int safe=0;
        for(int i=1;i<STEPS;i++) {
    ...
    public static synchronized Decision decide(String camera,String mode,long nowNs,
        ...
        boolean freshSample=valid&&consumedSampleNs!=sample.submittedNs;
        BodyCandidate proposedBody=valid&&sample.stats[0].fieldMapValid
        ...
        BodyCandidate multifieldBody=valid&&sample.stats[0].fieldMapValid
                ?stabilizeBodyCandidate(sample.stats[0],proposedBody,freshSample)
                :BodyCandidate.invalid("field_map_unavailable");
        int protectedOpenAnchorMask=valid&&sample.stats[0].fieldMapValid
                ?protectedOpenAnchorMask(sample.stats[0]):0;
        boolean protectedOpenAnchorSevere=valid&&protectedOpenAnchorMask!=0
                &&protectedOpenAnchorSevereBacklight(sample.stats[0],multifieldBody);
        boolean protectedOpenAnchorDeepBody=valid&&protectedOpenAnchorMask!=0
                &&protectedOpenAnchorDeepBodyAssist(sample.stats[0],multifieldBody);
        double protectedOpenAnchorLiftCap=valid
                ?protectedOpenAnchorLiftCap(sample.stats[0],multifieldBody):SEARCH_MAX_EV;
        double sceneLimit=valid?sceneKeyHeadroomLimitQualified(sample.stats,multifieldBody):0;
        boolean fieldMapOwnsClassification=valid&&sample.stats[0].fieldMapValid;
        boolean qualifiedMultifieldBody=fieldMapOwnsClassification&&multifieldBody.valid
                &&multifieldBody.confidence>=.20;
        double backlightEvidenceScore=valid&&qualifiedMultifieldBody
                ?backlightEvidenceScore(sample.stats[0],multifieldBody):0;
        double backlightEvidenceLiftCap=valid&&qualifiedMultifieldBody
                ?backlightEvidenceLiftCap(sample.stats[0],multifieldBody):SEARCH_MAX_EV;
        double highlightRetentionLimit=valid?highlightRetentionLimit(sample.stats):0;
        String sceneOwnership=fieldMapOwnsClassification
                ?(qualifiedMultifieldBody?"multifield_body_owns_backlight"
                    :"multifield_no_qualified_body_scene_fallback")
                :"legacy_center_backlight_fallback";
        double effectivePositiveLimit=0;
        double positiveRiseStepEv=NORMAL_POSITIVE_SLEW_EV;
        boolean fastAcquire=false;
        boolean safeFastAcquire075=false;
        ...
        if(manualExposure!=0||manualIso!=0) {
            ...
            sceneChosen=selectSceneKeyQualified(sample.stats,multifieldBody);
            ...
            // FINISH1K: upper backlight authority requires absolute bright-background
            // evidence, then every positive branch respects the global emerging-
            // highlight retention ceiling. Both are global capture-exposure limits.
            if(useBacklight&&qualifiedMultifieldBody)
                positiveLimit=Math.min(positiveLimit,backlightEvidenceLiftCap);
            positiveLimit=Math.min(positiveLimit,highlightRetentionLimit);
            if(protectedOpenAnchorMask!=0)
                positiveLimit=Math.min(positiveLimit,protectedOpenAnchorLiftCap);
            effectivePositiveLimit=positiveLimit;
            ...
            safeFastAcquire075=safeFastAcquire075Eligible(sample.stats[0],multifieldBody,
                    useBacklight,useScene,protectedOpenAnchorMask,target,current,
                    Math.max(0,nowNs-sample.submittedNs));
            fastAcquire=safeFastAcquire075||fastAcquireEligible(sample.stats[0],multifieldBody,
                    useBacklight,useScene,protectedOpenAnchorMask,
                    protectedOpenAnchorSevere,target,current);
            ...
            if(useBacklight) {
                if(backlightRequest>effectivePositiveLimit+1e-9) {
                    if(protectedOpenAnchorMask!=0
                            &&protectedOpenAnchorLiftCap<=effectivePositiveLimit+1e-9
                            &&protectedOpenAnchorLiftCap<backlightRequest-1e-9)
                        reason=protectedOpenAnchorDeepBody
                                ?"rendered_open_anchor_deep_body_cap"
                                :"rendered_open_anchor_cap";
                    else if(qualifiedMultifieldBody
                            &&backlightEvidenceLiftCap<=effectivePositiveLimit+1e-9
                            &&backlightEvidenceLiftCap<backlightRequest-1e-9)
                        reason="rendered_backlight_background_evidence_cap";
                    else if(highlightRetentionLimit<=effectivePositiveLimit+1e-9
                            &&highlightRetentionLimit<backlightRequest-1e-9)
                        reason="rendered_global_highlight_retention_cap";
                    else
                        reason="rendered_multifield_m9_character_or_headroom";
                } else if(multifieldBody.valid) {
        ...
        try {
            o.put("revision",REVISION).put("reason",reason)
                    ...
                    .put("sceneOwnership",sceneOwnership)
                    .put("fieldMapOwnsClassification",fieldMapOwnsClassification)
                    .put("qualifiedMultifieldBodyOwnsBacklight",qualifiedMultifieldBody)
                    .put("backlightEvidenceQualificationRevision","BGQUAL1A")
                    .put("backlightEvidenceScore",backlightEvidenceScore)
                    .put("backlightEvidenceWeakBackgroundMaxEv",WEAK_BACKGROUND_MAX_EV)
                    .put("backlightEvidenceLiftCapEv",backlightEvidenceLiftCap)
                    .put("highlightRetentionRevision","HIGHLIGHTRET1B_PREEXISTING_Q90")
                    .put("highlightRetentionPolicy",
                            "global_single_capture_exposure_incremental_emerging_highlights_no_HDR")
                    .put("highlightRetentionLimitEv",highlightRetentionLimit)
                    .put("highlightRetentionPreexistingFieldQ90Code",
                            PREEXISTING_HIGHLIGHT_Q90_CODE)
                    .put("localHdrUsed",false)
                    .put("multiFrameHdrUsed",false)
                    .put("fastAcquireEligible",fastAcquire)
                    .put("safeFastAcquire075Eligible",safeFastAcquire075)
                    .put("positiveRiseStepEv",positiveRiseStepEv)
                    .put("normalPositiveSlewEv",NORMAL_POSITIVE_SLEW_EV)
                    .put("fastPositiveSlewEv",FAST_POSITIVE_SLEW_EV)
                    .put("safeFastPositiveSlewEv",SAFE_FAST_POSITIVE_SLEW_EV)
                    .put("safeFastMaxSampleAgeMs",SAFE_FAST_MAX_SAMPLE_AGE_NS/1e6)
                    ...
                    .put("multifieldProposedBody",proposedBody.json())
                    .put("multifieldBody",multifieldBody.json())
                    .put("bodyLockActive",latchedBodyMask!=0)
                    .put("bodyLockMask",latchedBodyMask)
                    .put("bodyLockTargetMedianCode",latchedBodyTargetMedian)
                    .put("bodyLockTargetQ25Code",latchedBodyTargetQ25)
                    .put("bodyLockReason",bodyLockReason)
                    ...
                    .put("protectedOpenAnchorMask",protectedOpenAnchorMask)
                    .put("protectedOpenAnchorFieldCount",
                            valid&&sample.stats[0].fieldMapValid
                                    ?Integer.bitCount(protectedOpenAnchorMask):0)
                    .put("protectedOpenAnchorPolicy","soft_positive_lift_cap")
                    .put("protectedOpenAnchorSevereBacklight",protectedOpenAnchorSevere)
                    .put("protectedOpenAnchorDeepBodyAssist",protectedOpenAnchorDeepBody)
                    .put("protectedOpenAnchorDeepBodyMaxEv",PROTECTED_ANCHOR_DEEP_BODY_MAX_EV)
                    .put("protectedOpenAnchorLiftCapEv",protectedOpenAnchorLiftCap)
                    .put("effectivePositiveLimitEv",effectivePositiveLimit)
                    ...
                    .put("backgroundLossPolicy",
                            "multifield_body_background_qualified_then_incremental_highlight_retention")
    ...
    private static BodyCandidate stabilizeBodyCandidate(
            Stats base,BodyCandidate proposed,boolean freshSample) {
        ...
        if(latchedBodyMask==0) {
            if(proposed.valid&&proposed.confidence>=.22) {
                latchBody(proposed);
                bodyLockReason="engaged";
                return candidateForMask(base,latchedBodyMask,
                        latchedBodyTargetMedian,latchedBodyTargetQ25,latchedBodySeverity,
                        "latched_body_engaged");
            }
            bodyLockReason="awaiting_confident_body";
            return proposed;
        }

        BodyCandidate held=candidateForMask(base,latchedBodyMask,
                latchedBodyTargetMedian,latchedBodyTargetQ25,latchedBodySeverity,
                "latched_body_held");
        boolean heldPlausible=held.valid&&held.confidence>=.08;

        if(heldPlausible) {
            if(proposed.valid) {
                // A current coherent candidate is real scene evidence. It clears
                // candidate-absence misses even when spatial hysteresis keeps the old mask.
                bodyMissConfirmations=0;
                double overlap=maskOverlap(latchedBodyMask,proposed.mask);
                if(overlap>=.50) {
                    pendingBodyMask=0;pendingBodyConfirmations=0;
                    bodyLockReason="held_overlap";
                    return held;
                }

                // A completely different candidate has to be materially convincing
                // for two fresh meter samples before it can replace a stable body.
                if(proposed.confidence>=Math.max(.22,held.confidence+.08)) {
                    if(freshSample) {
                        if(maskOverlap(pendingBodyMask,proposed.mask)>=.50) {
                            pendingBodyConfirmations++;
                        } else {
                            pendingBodyMask=proposed.mask;
                            pendingBodyConfirmations=1;
                        }
                        if(pendingBodyConfirmations>=2) {
                            latchBody(proposed);
                            bodyLockReason="switched_after_two_confirmations";
                            return candidateForMask(base,latchedBodyMask,
                                    latchedBodyTargetMedian,latchedBodyTargetQ25,
                                    latchedBodySeverity,"latched_body_switched");
                        }
                    }
                    bodyLockReason="holding_old_body_pending_switch";
                } else {
                    pendingBodyMask=0;pendingBodyConfirmations=0;
                    bodyLockReason="held_stronger_than_new_body";
                }
            } else {
                pendingBodyMask=0;pendingBodyConfirmations=0;

                // BODYLOCKRELEASE1A: the old spatial mask may remain locally dark
                // after the scene no longer contains a coherent body. Previously that
                // local plausibility reset the miss counter forever, making Auto exposure
                // history-dependent. Allow one fresh detector miss for meter noise, then
                // release on the second consecutive fresh miss. Reusing the same meter
                // sample cannot advance the release counter.
                if(freshSample) bodyMissConfirmations++;
                if(bodyMissConfirmations>=2) {
                    resetBodyLock();
                    bodyLockReason="released_after_two_candidate_misses";
                    return BodyCandidate.invalid("body_lock_released_candidate_absent");
                }
                bodyLockReason="held_one_candidate_miss";
            }
            return held;
        }

        // The old mask no longer describes a credible dark body. Avoid using it
        // for exposure, but require two fresh samples before adopting a disjoint one.
        if(freshSample)bodyMissConfirmations++;
        if(proposed.valid&&proposed.confidence>=.22) {
            if(freshSample) {
                if(maskOverlap(pendingBodyMask,proposed.mask)>=.50) {
                ...
                if(pendingBodyConfirmations>=2) {
                    latchBody(proposed);
                    bodyLockReason="reacquired_after_scene_change";
                    return candidateForMask(base,latchedBodyMask,
                            latchedBodyTargetMedian,latchedBodyTargetQ25,
                            latchedBodySeverity,"latched_body_reacquired");
                }
            }
            bodyLockReason="old_body_invalid_pending_reacquire";
            return BodyCandidate.invalid("body_lock_reacquire_pending");
        }

        if(bodyMissConfirmations>=2) {
            resetBodyLock();
            bodyLockReason="released_after_two_misses";
        } else {
            bodyLockReason="old_body_invalid_first_miss";
        }
        return BodyCandidate.invalid("body_lock_invalid");
    }

    private static BodyCandidate candidateForMask(Stats s,int mask,int targetMedian,
            int targetQ25,double severity,String reason) {
        if(s==null||!s.valid()||!s.fieldMapValid||mask==0)
            return BodyCandidate.invalid("latched_mask_unavailable");
        BodyMetrics body=bodyMetrics(s,mask);
        if(body.q90<16||body.median>=90)
            return BodyCandidate.invalid("latched_mask_no_longer_dark_body");
        double contrast=body.backgroundQ90-body.median;
        double need=Math.max(smoothstep(70.-body.median,4,40),
                smoothstep(40.-body.q25,2,28));
        double support=Math.max(smoothstep(contrast,24,90),
                smoothstep(body.backgroundBright,.06,.28));
        ...
        if(confidence<.05)
            return BodyCandidate.invalid("latched_mask_confidence_lost");
        int count=Integer.bitCount(mask);
        return new BodyCandidate(true,reason,mask,count,confidence,severity,
                body.median,body.q25,body.q90,body.bright,body.clipped,
                body.backgroundQ90,body.backgroundBright,targetMedian,targetQ25);
    }

    private static void latchBody(BodyCandidate body) {
        latchedBodyMask=body.mask;
        latchedBodyTargetMedian=body.targetMedian;
        latchedBodyTargetQ25=body.targetQ25;
        latchedBodySeverity=body.severity;
        pendingBodyMask=0;pendingBodyConfirmations=0;bodyMissConfirmations=0;
    }

    private static void resetBodyLock() {
        latchedBodyMask=0;latchedBodyTargetMedian=0;latchedBodyTargetQ25=0;
        latchedBodySeverity=0;pendingBodyMask=0;pendingBodyConfirmations=0;
        bodyMissConfirmations=0;bodyLockReason="none";
    }

    private static double maskOverlap(int a,int b) {
        if(a==0||b==0)return 0;
        int shared=Integer.bitCount(a&b);
        int smaller=Math.min(Integer.bitCount(a),Integer.bitCount(b));
    ...
    /**
     * 1G middle ground: a protected open anchor constrains, rather than disables,
     * the dark-body exposure path. Severe backlight requires both a genuinely
     * starved coherent body and strong near-white/clipped background evidence.
     */
    public static boolean protectedOpenAnchorSevereBacklight(Stats s,BodyCandidate body) {
        if(s==null||!s.valid()||!s.fieldMapValid||body==null||!body.valid
                ||protectedOpenAnchorMask(s)==0)return false;
        BodyMetrics m=bodyMetrics(s,body.mask);
        double contrast=m.backgroundQ90-m.median;
        boolean brightBackground=s.outerBright>=.10||s.outerClipped>=.04
                ||s.bright>=.08;
        return body.severity>=.70&&contrast>=100&&brightBackground;
    }

    public static boolean protectedOpenAnchorDeepBodyAssist(Stats s,BodyCandidate body) {
        if(s==null||!s.valid()||!s.fieldMapValid||body==null||!body.valid
                ||protectedOpenAnchorMask(s)==0)return false;
        // This is deliberately narrower than "severe backlight": it is for a
        // truly black coherent body beside an open textured region when the rest
        // of the frame is not itself strongly bright. The newer BGQUAL1A score
        // must remain weak so this cannot become an alternate route to +2.5 EV.
        return body.confidence>=.35&&body.severity>=.85
                &&body.bodyMedian<=18&&body.bodyQ25<=8
                &&backlightEvidenceScore(s,body)<=.15;
    }

    public static double protectedOpenAnchorLiftCap(Stats s,BodyCandidate body) {
        if(s==null||!s.valid()||!s.fieldMapValid||protectedOpenAnchorMask(s)==0)
            return SEARCH_MAX_EV;
        if(protectedOpenAnchorSevereBacklight(s,body))
            return PROTECTED_ANCHOR_SEVERE_MAX_EV;
        if(protectedOpenAnchorDeepBodyAssist(s,body))
            return PROTECTED_ANCHOR_DEEP_BODY_MAX_EV;
        return PROTECTED_ANCHOR_ORDINARY_MAX_EV;
    }

    /**
     * FINISH1K background qualification.
     *
     * A coherent dark body earns up to +1 EV from contrast/starvation alone.
     * Additional positive authority is unlocked only by absolute evidence that
     * the non-body part of the frame is genuinely bright. The score is deliberately
     * semantic-free and uses only the same neutral-reference rendered meter.
     */
    public static double backlightEvidenceScore(Stats s,BodyCandidate body) {
        if(s==null||!s.valid()||!s.fieldMapValid||body==null||!body.valid)return 0;
        BodyMetrics m=bodyMetrics(s,body.mask);
        double q90Support=smoothstep(m.backgroundQ90,110,190);
        double brightFieldSupport=smoothstep(m.backgroundBright,.04,.28);
        double outerBrightSupport=smoothstep(s.outerBright,.025,.16);
        double outerClipSupport=smoothstep(s.outerClipped,.005,.06);
        return clamp(Math.max(Math.max(q90Support,brightFieldSupport),
                Math.max(outerBrightSupport,outerClipSupport)),0,1);
    }

    public static double backlightEvidenceLiftCap(Stats s,BodyCandidate body) {
        if(s==null||!s.valid()||!s.fieldMapValid||body==null||!body.valid)
            return SEARCH_MAX_EV;
        double score=backlightEvidenceScore(s,body);
        double cap=WEAK_BACKGROUND_MAX_EV
                +(SEARCH_MAX_EV-WEAK_BACKGROUND_MAX_EV)*score;
        // Quantise down to the same 0.25-EV search lattice so the cap cannot
        // introduce a new fractional exposure state or preview/capture mismatch.
        return clamp(Math.floor((cap+1e-9)/STEP_EV)*STEP_EV,
                WEAK_BACKGROUND_MAX_EV,SEARCH_MAX_EV);
    }

    /**
     * Global single-exposure highlight retention. This is not HDR: no local
     * relighting, tone-map mask, multi-frame merge or shadow/highlight split is
     * introduced. We only stop increasing capture exposure when previously
     * non-bright rendered fields are being driven into near-white/clipping.
     *
     * Existing bright/clipped windows are intentionally grandfathered by using
     * growth from the neutral-reference baseline; a pre-blown window therefore
     * does not automatically veto useful backlight assistance.
     */
    public static boolean highlightRetentionUnsafe(Stats base,Stats v) {
        ...
        for(int f=0;f<FIELD_COUNT;f++) {
            ...
            boolean preexistingHighlight=
                    base.fieldQ90[f]>=PREEXISTING_HIGHLIGHT_Q90_CODE;
            if(!preexistingHighlight&&base.fieldBright[f]<.20&&v.fieldBright[f]>=.55
                    &&fieldBrightGrowth>=.30)newNearWhiteFields++;
            if(!preexistingHighlight&&base.fieldClipped[f]<.04&&v.fieldClipped[f]>=.08
    ...
    public static double highlightRetentionLimit(Stats[] s) {
        if(!validStats(s))return 0;
        Stats base=s[0];int safe=0;
        for(int i=1;i<STEPS;i++) {
            if(highlightRetentionUnsafe(base,s[i]))break;
            safe=i;
        }
        return ev(safe);
    }
    ...
    private static boolean sceneKeyStarved(Stats s,BodyCandidate body) {
        if(s==null||!s.valid()||s.median>=42||s.centerMedian>=56||s.centerQ25>=34)
            return false;
        // FIELDOWNERSHIP1A: when the 4x6 rendered map is available it owns the
        // backlight-vs-scene decision. A legacy centre/surround contrast signal
        // cannot block whole-scene placement unless a coherent 4x6 body actually
        // qualifies. This closes the "neither branch fires" gap in canopy scenes.
        if(s.fieldMapValid)
            return body==null||!body.valid||body.confidence<.20;
        return backlightConfidence(s)<.20;
    }

    private static boolean safeFastAcquire075Eligible(Stats s,BodyCandidate body,
            boolean useBacklight,boolean useScene,int openAnchorMask,
            double target,double current,long sampleAgeNs) {
        if(s==null||!s.valid()||sampleAgeNs>SAFE_FAST_MAX_SAMPLE_AGE_NS
                ||openAnchorMask!=0||target-current<1.25-1e-9)
            return false;
        if(useScene) {
            // SAFE075 is deliberately narrower than the 0.50-EV fast path.
            // It requires a profoundly starved scene and leaves at least 0.50 EV
            // between the 0.75-EV step and the already-rendered safe target.
            return s.median<=20&&s.centerMedian<=30&&s.centerQ25<=14&&s.dark>=.65;
        }
        if(!useBacklight||body==null||!body.valid)return false;
        return body.confidence>=.50&&body.severity>=.75;
    }

    private static boolean fastAcquireEligible(Stats s,BodyCandidate body,
            boolean useBacklight,boolean useScene,int openAnchorMask,
            boolean openAnchorSevere,double target,double current) {
        if(s==null||!s.valid()||target-current<FAST_POSITIVE_SLEW_EV-1e-9)
            return false;
        if(useScene) {
            // Whole-scene fast acquisition is restricted to an unambiguously
            // starved rendered frame. The final target and all highlight limits
            // remain unchanged; only convergence cadence is faster.
            return s.median<=28&&s.centerMedian<=40&&s.centerQ25<=20&&s.dark>=.55;
        }
        if(!useBacklight)return false;
        if(openAnchorMask!=0&&!openAnchorSevere)return false;
        if(body!=null&&body.valid)
            return body.confidence>=.35&&body.severity>=.55;
        // Legacy fallback is retained only for devices without a valid field map.
        return !s.fieldMapValid&&backlightConfidence(s)>=.45
                &&backlightPlacementSeverity(s)>=.65;
    }
