#!/usr/bin/env python3
"""Preserve M9 exposure energy across gyro and ISO limits on GL2F."""
from pathlib import Path
import hashlib,json,subprocess,sys
root=Path(sys.argv[1]).resolve();here=Path(__file__).resolve().parent
m=json.loads((here/'m9cam-m9livegl2g-manifest.json').read_text())
for rel,item in m['changed'].items():
 p=root/rel;actual=hashlib.sha256(p.read_bytes()).hexdigest() if p.exists() else None
 if actual!=item['before']:raise SystemExit('GL2F baseline mismatch: '+rel)
...
patch=here/'m9cam-m9livegl2g.patch'
subprocess.run(['git','-C',str(root),'apply','--check',str(patch)],check=True)
subprocess.run(['git','-C',str(root),'apply',str(patch)],check=True)
subprocess.run([sys.executable,str(here/'verify-m9cam-m9livegl2g.py'),str(root)],check=True)
