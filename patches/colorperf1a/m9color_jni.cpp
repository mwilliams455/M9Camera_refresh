...
//
// This function is append-only relative to PERF3I. The Java caller falls back to
// the retained PERF3I block loop if validation or Bitmap locking fails before any
...
Java_com_particlesdevs_photoncamera_m9_render_M9NativeColorCore_renderFramePersistentDirectBitmap(
    ...
    const auto nativeStarted = std::chrono::steady_clock::now();
    const int boundedRows = std::min(static_cast<int>(blockRows), static_cast<int>(sourceHeight));
    const int workerCount = std::max(1, std::min(static_cast<int>(workers), boundedRows));
    ...
    std::vector<std::thread> threads;
    threads.reserve(static_cast<size_t>(workerCount));
    M9ColorReusableBarrier barrier(workerCount);
    ...
    for (int worker = 0; worker < workerCount; ++worker) {
        threads.emplace_back([&, worker, camBase, argbBase, bitmapBase]() {
            ...
            for (int blockY0 = 0; blockY0 < sourceHeight; blockY0 += blockRows) {
                const int rows = std::min(static_cast<int>(blockRows),
                                          static_cast<int>(sourceHeight) - blockY0);
                const int y0 = (rows * worker) / workerCount;
                const int y1 = (rows * (worker + 1)) / workerCount;
                ...
                if (y1 > y0) {
                    const int localRows = y1 - y0;
                    const int localPixels = localRows * width;
                    const size_t localPixelOffset = static_cast<size_t>(y0) * static_cast<size_t>(width);
                ...
                // No worker may reuse the bounded block scratch until every worker
                // has completed its exact destination copy for the current block.
                barrier.wait();
            }
            ...
            combinedElapsedNs[static_cast<size_t>(worker)] = static_cast<int64_t>(
                    std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    const auto outputCopyStarted = std::chrono::steady_clock::now();
    const auto outputCopyEnded = outputCopyStarted;
    const int unlockResult = AndroidBitmap_unlockPixels(env, bitmap);
    const auto nativeEnded = std::chrono::steady_clock::now();
    if (unlockResult != ANDROID_BITMAP_RESULT_SUCCESS) {
    ...
    const jlong stats[12] = {
            ...
            static_cast<jlong>(orientationNsSum),
            static_cast<jlong>(std::chrono::duration_cast<std::chrono::nanoseconds>(
                    outputCopyEnded - outputCopyStarted).count()),
            static_cast<jlong>(std::chrono::duration_cast<std::chrono::nanoseconds>(
