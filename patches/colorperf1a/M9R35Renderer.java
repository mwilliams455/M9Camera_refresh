...
public final class M9R35Renderer {
    ...
    private static RenderCore renderCore(ByteBuffer rawBuffer,
        ...
        try {
            ...
            final int nativeColorBlockCount =
                    (height + NATIVE_COLOR_BLOCK_ROWS - 1) / NATIVE_COLOR_BLOCK_ROWS;
            boolean nativeColorPersistentFrameActive = false;

            // COLORPERF1A first tries one native call for the whole frame. Native keeps
            // the exact 384-row bounded scratch geometry and the exact PERF3I per-block
            // worker row partitions, but reuses one worker team and one Bitmap lock.
            // A false return is guaranteed to occur before pixel mutation.
            if (nativeColorCvDirectEligible && nativeColorBitmapDirectEligible) {
                final long nativeStartedNs = System.nanoTime();
                ...
                if (nativeColorPersistentFrameActive) {
                    ...
                    nativeColorCvDirectBlocks = nativeColorBlockCount;
                    nativeColorBitmapDirectBlocks = nativeColorBlockCount;
                }
            }

            // Exact promoted fallback: retained byte-for-byte block loop from PERF3I.
            if (!nativeColorPersistentFrameActive) {
