...
extern "C" int trial_reconstruct_perf(const uint16_t* raw,const float* variance,const uint8_t* censored,
 ...
 try {
  ...
  if(variance){
   corrected.resize(count);
   auto t0=Clock::now();
   int rc=phase_noise_symtile_adaptive_exact(raw,variance,censored,w,h,corrected.data(),workers);if(rc)return rc;
...
// AMAZEBANDPERF1A host research entry point.
//
// This duplicates trial_reconstruct_perf after the frozen phase-noise,
// quarter-blend and float-input stages, with bandRows supplied by the caller.
// It exists only to prove whether changing the bounded AMaZE destination window
// is byte-exact. Production remains 256 until a candidate passes every gate.
extern "C" int trial_reconstruct_perf_band(const uint16_t* raw,const float* variance,const uint8_t* censored,
 int w,int h,int cfa,double nr,double nb,double scale,uint16_t* out,int workers,double* stats,double* perf,
 int bandRows) {
 if(!raw||!out||!stats||!perf||w<64||h<64||w>8192||h>8192||cfa<0||cfa>3||workers<1||workers>8||
    bandRows<128||bandRows>2048||(bandRows%128)!=0||
    !std::isfinite(nr)||!std::isfinite(nb)||nr<.0625||nb<.0625||nr>16||nb>16||
    !std::isfinite(scale)||scale<=0||scale>64||(bool(variance)!=bool(censored)))return -1;
 std::fill(stats,stats+4,0.);std::fill(perf,perf+5,0.);
 using Clock=std::chrono::steady_clock;
...
extern "C" const char* m9_amazebandperf1a_research_revision(){
 return "M9AMAZEBANDPERF1A_SWEEP_ONLY";
}
...
extern "C" const char* m9_phasenoiseperf1c_reconstruct_revision(){return "M9PHASENOISEPERF1C_ADAPTIVE128_EXACT";}
