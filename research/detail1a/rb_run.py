#!/usr/bin/env python3
"""DETAIL1C: fixed-Sharp comparison of partial recovered R/B reconstruction."""
...
from rb_probe import RbProbe,checks,q14

NAMES=['baseline','carrier_no_shrink','carrier_co2_probe']
LABELS=['Current MHC differences','Recovered carrier, no shrink','Recovered carrier + Co=2']
...
def comparison(meta,decoded):
    ...
    d.text((12,12),f'M9 DETAIL1C | {model} | ISO {meta["iso"]} | fixed ISO160 Sharp | decoded JPEG, 100%',font=title,fill='white')
    ...
    d.text((12,920),'Partial reconstruction probes; Noise2 bypassed. Same green, colour transform and JPEG95 encoder.',font=small,fill='#bdc8cd')
    return panel
...
def main():
    ...
    colour=Downstream(repo,a.assembled.resolve(),out/'native_colour');probe=RbProbe(out/'native')
    report=dict(schema='m9.detail1c.rb.partial.v1',source_hashes=colour.hashes,checks=checks(probe),captures=[],
        scope='Recovered pre-Noise difference shrink and opposite-phase carrier, plus final R/B consumer; fixed GL2G green and Sharp',
        boundaries=['Noise2 is bypassed, not implemented; not full Leica reconstruction parity',
            ...
            'Camera-native NORM030 raw adapted to recovered 14-bit equations; Leica upstream WB placement not established',
    ...
    for path in a.dngs:
        ...
        stages=probe.stages(raw,cfa,False)
        ...
        unshrunk_diff=stages[1][native_mask].copy();border=np.ones((h,w),bool);border[10:-10,10:-10]=False
        for k,name in enumerate(NAMES):
            if k==2:stages=probe.stages(raw,cfa,True)
            cam=base if k==0 else probe.consume(stages[2],base,cfa)
        ...
        print(json.dumps(dict(iso=meta['iso'],name=path.name,gain_delta_ev=[v['gain_delta_ev'] for v in meta['variants']],
            shrink_pct=meta['variants'][2]['native']['native_difference_shrunk_pct'])),flush=True)
